...
# deploy script
# assumes that industrycoin repo lives at $HOME/industrycoin
# and that industrycoin-37 is the virtualenv under which it lives

# setup
cd || echo "Cannot find directory!"
cd industrycoin/coin || echo "Cannot find coin directory!"
...
source ../industrycoin-37/bin/activate
...
rm -Rf ~/industrycoin/coin/app/static/other
...
if [ "$ISFRONTENDPUSH" ] && [ "$JOBS_NODE" ]; then
    if [ "$DISTID" ]; then
        cd ~/industrycoin/coin || echo "Cannot find coin directory!"; bash scripts/bustcache.bash "$DISTID"
    fi
fi
