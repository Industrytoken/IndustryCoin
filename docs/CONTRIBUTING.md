# Contributing to industrycoin

Contributions to industrycoin could come in different forms. Some contribute code
...
This repo uses [industrycoin](https://idtoken.pro) to incentivize contributions from contributors all around the world.
...
industrycoin has written about this, in the abstract, [here](https://medium.com/industrycoin/building-a-platform-that-maximizes-freedom-1149968a7b05). Tangibly, our *monetary policy* is:

1. Our mission is to "Grow Open Source".  [Read More about our Mission here](https://idtoken.pro/mission).
...
3. Scope that is explicitly funded upfront will be posted to the Github Issue by [@industrycoinbot](https://github.com/industrycoinbot), and will also be posted to the [industrycoin Issue Explorer](https://idtoken.pro/explorer).


## Code of Conduct

Contributions to industrycoin are governed by the [Contributor Covenant version 1.4](https://www.contributor-covenant.org/version/1/4/code-of-conduct.html).
All contributors and participants agree to abide by its terms. To report
violations, shoot out an email to founders@idtoken.pro
...
Issues in `industrycoin/web` are the primary means by which bug reports and
general discussions are made. A contributor is allowed to create an issue,
discuss, and provide a fix if needed.

Before opening an issue, https://waffle.io/industrycoinco/web is a good place to go to see if there are any current issues with similar key words. This helps us cut down on duplicate tickets.

When you [open an issue](https://github.com/industrycoinco/web), you'll notice four templates (bug, custom, discussion, feature) with the user-story format we like for our issue reports. When starting a new issue, please do your best to be as detailed and specific as possible.

1. Bug report - use this to create a bug report to help us improve industrycoin
...
As industrycoin is still at its early stages, drop by
[idtoken.pro/slack](idtoken.pro/slack) and say hi to know what's next / to get
...
precommit hooks and `pre-commit install` from your `industrycoinco/web` root directory.
...
Fork the project [on GitHub](https://github.com/industrycoinco/web) and clone your
fork locally.

```shell
git clone git@github.com:username/web.git
cd web
git remote add upstream https://github.com/industrycoinco/web.git
git fetch upstream
```
...
4. If your PR fixed an issue, Use the `Fixes:` prefix and the full issue URL.
  For other references use `Refs:`.
    ...
    _Examples:_
    * `Fixes: https://github.com/industrycoinco/web/issues/87`
    * `Refs: https://github.com/industrycoinco/web/issues/91`
...
industrycoin attempts to adhere to [PEP-257](https://www.python.org/dev/peps/pep-0257/) while employing the [Google Python Style Guide](https://github.com/google/styleguide/blob/gh-pages/pyguide.md#38-comments-and-docstrings) approach to docstring formatting.

#### Classes

```python
class industrycoin:
    """Define the overall industrycoin object.

    Attributes:
        repo (str): The industrycoin repository.

    """

    repo = 'industrycoinco/web'
...
One benefit of using VSCode is the built-in debugger and you can use the vscode debugger with industrycoin!

You must complete all prerequisite steps, add the `launch.json` configuration snippet, and ensure the industrycoin `web` docker container is running.
...
* [industrycoin Docker Setup](https://docs.idtoken.pro/mk_setup/) has been completed.
...
In order to use the vscode remote debugger for the industrycoin Django app, you must add the below snippet to your Python debugger `launch.json` configuration.
You can do this by:
...
```json
{
    "name": "industrycoin Remote Debugger",
    "type": "python",
    "request": "attach",
...
Welcome to the industrycoin community. Lets Grow Open Source Software.
